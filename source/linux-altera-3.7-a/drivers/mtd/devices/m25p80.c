...
#define   OPCODE_RDFLGSR     0x70  /* Read flag status register */
...
#define   FLG_SR_READY		0x80	/*program,erase is ready*/
...
static int read_flag_sr(struct m25p *flash)
{
	ssize_t retval;
	u8 code = OPCODE_RDFLGSR;
	u8 val;
...
static int wait_till_program_erase_ready(struct m25p *flash)
{
	unsigned long deadline;
	int flag_sr;
	...
	do {
		flag_sr = read_flag_sr(flash);
		if (flag_sr < 0)
			break;
		else if (flag_sr & FLG_SR_READY)
			return 0;
...
static int erase_sector(struct m25p *flash, u32 offset)
{
	...
	/* Wait until finished erase command. */
	if (wait_till_program_erase_ready(flash)) //xuewt add
	{
		pr_err("spi nor flash program timeout!\n");
	...
	size_t *retlen, const u_char *buf)
{
	...
	/* Wait until finished previous write command. */
	if (wait_till_ready(flash)) {
		pr_err("spi nor flash previous write command in progress!can not issue write command again\n");
		mutex_unlock(&flash->lock);
		return 1;
	}
	...
	if (page_offset + len <= flash->page_size) {
		...
		/* Wait until finished program command. */
		if (wait_till_program_erase_ready(flash)) //xuewt add
		{
			pr_err("spi nor flash program timeout!\n");
			mutex_unlock(&flash->lock);
...
static const struct spi_device_id m25p_ids[] = {
	...
	/* Micron/Numonyx */
	{ "n25q00",  INFO(0x20ba21, 0, 64 * 1024, 2048, 0) },
