...
static int register_root_hub(struct usb_hcd *hcd)
{
	...
	usb_dev->devnum = devnum;			/* root hub的地址始终为1  */
	usb_dev->bus->devnum_next = devnum + 1;
	memset (&usb_dev->bus->devmap.devicemap, 0,
			sizeof usb_dev->bus->devmap.devicemap);
	set_bit (devnum, usb_dev->bus->devmap.devicemap);
	usb_set_device_state(usb_dev, USB_STATE_ADDRESS);/* root hub 进入地址态 */

	mutex_lock(&usb_bus_list_lock);

	usb_dev->ep0.desc.wMaxPacketSize = cpu_to_le16(64);
	retval = usb_get_device_descriptor(usb_dev, USB_DT_DEVICE_SIZE);/* 标准设备描述符长度是18个字节 */
		...
		unsigned int irqnum, unsigned long irqflags)
{
	...
	if ((retval = register_root_hub(hcd)) != 0)/* 注册root hub时会获取root hub的端点描述符 */
