...
void dwc_modify_reg32(uint32_t volatile *addr, uint32_t clear_mask, uint32_t set_mask)
{
	uint32_t v = *addr;
                                                      ...
                                                      _core_if, int hc_num)
{
        return (dwc_read_reg32
                (&_core_if->host_if->hc_regs[hc_num]->hcint));
}
...
int dwc_otg_set_param_phy_type(dwc_otg_core_if_t * core_if, int32_t val)
{
	int retval = 0;
	int valid = 0;
	printf("dwc_otg_set_param_phy_type:%d\n", val);
	if (DWC_OTG_PARAM_TEST(val, 0, 2)) {
...
#if 0
	/* Power the PHY*/
        hprt00 = dwc_otg_read_hprt0(core_if);
        hprt00 = DWC_HPRT0_PRT_PWR_RW(hprt00, 1);
        dwc_write_reg32(core_if->host_if->hprt0, hprt00);

        /* Reset the PHY */
        hprt00 = dwc_otg_read_hprt0(core_if);
        hprt00 = DWC_HPRT0_PRT_RST_RW(hprt00, 1);
        dwc_write_reg32(core_if->host_if->hprt0, hprt00);
        ...
        hprt00 = dwc_otg_read_hprt0(core_if);
        hprt00 = DWC_HPRT0_PRT_RST_RW(hprt00, 0);
        dwc_write_reg32(core_if->host_if->hprt0, hprt00);
#endif
...
void dwc_otg_core_init(dwc_otg_core_if_t * core_if)
{
	...
	if ((core_if->core_params->speed == DWC_SPEED_PARAM_FULL) &&
		...
		if (!core_if->phy_init_done) {
			...
			printf("usbcfg.b.ulpi_utmi_sel:%d\n", usbcfg.b.ulpi_utmi_sel);
			//if (usbcfg.b.ulpi_utmi_sel == 1) { //yejc
