...
#define CONFIG_SYS_USB1_BASE 0xffb40000
...
#define internal_base CONFIG_SYS_USB1_BASE
...
int dwc_host_init(void)
{
	if (wait_for_connection())
		return 1<<2;
	printf("wait_for_connection pass\n");
	if (port_reset())
		return 1<<3;
	printf("port_reset pass\n");
	if (port_enabled())
		return 1<<5;
	printf("port enable pass\n");
	if (port_speed(DWC_HPRT_PRTSPD_HS))
		return 1<<4;
	printf("port_speed pass\n");
	if (wait_for_debounce())
		return 1<<4;
	printf("wait_for_debounce pass\n");
	return 0;
}
...
static int ctrlreq_setup_phase(struct usb_device *dev,
	...
	printf("ctrlreq_setup_phase:++wait for xfer complete\n");
...
static int ctrlreq_in_data_phase(struct usb_device *dev, unsigned long pipe,
	...
	while (rxlen < len) {
		...
		rxlen += nextlen;
		printf("ctrlreq_in_data_phase:rxlen=0x%08x\n", rxlen);
		dev->act_len = rxlen;
	}
	printf("ctrlreq_in_data_phase break\n");
	return 0;
}
...
static int ctrlreq_in_status_phase(struct usb_device *dev, unsigned long pipe)
{
	...
	printf(" ctrlreq_in_status_phase:++wait ack");
