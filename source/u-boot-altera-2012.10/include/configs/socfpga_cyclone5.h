...
#elif (CONFIG_PRELOADER_BOOT_FROM_QSPI == 1)
/* 
  * If not run "mmc rescan", kernel boot from the qspi will not detect the sd normally 
  */
#define CONFIG_BOOTCOMMAND "mmc rescan;run qspiload; run qspiboot"
#else
#define CONFIG_BOOTCOMMAND "run mmcload; run mmcboot"
#endif
#endif
...
*                        www.embest-tech.com
***************************************************************************
* The L4 OSC1 Timer1 is used for testing BM load time
*/

/*
  * L4 OSC1 Timer1
 */
#define CONFIG_USE_OSC1_TIMER1
#define CONFIG_OSC1_TIMER1_BASE	(0xffd01000)
...
*                        www.embest-tech.com
***************************************************************************
*Because the sd card driver has used the dma in the 2013.01, and the speed promted greatly.
*so we have ported it to here.
*
*/
...
*                        www.embest-tech.com
***************************************************************************
* Because the qspi driver will auto probe the read data capture delay in the u-boot-2013.01
* so we port it to here.
*
*/
...
/* QSPI Delay timing */
#define CONFIG_CQSPI_TSHSL_NS		(200)	/* bit24 */
#define CONFIG_CQSPI_TSD2D_NS		(255)	/* bit16 */
#define CONFIG_CQSPI_TCHSH_NS		(20)		/* bit8 */
#define CONFIG_CQSPI_TSLCH_NS		(20)		/* bit0 */
...
***************************************************************************
*
*Support for amp bm for sd card
*/
#ifndef CONFIG_PRELOADER_BOOT_FROM_SDMMC
#error "CONFIG_PRELOADER_BOOT_FROM_SDMMC must be defined"
#endif
#if (CONFIG_PRELOADER_BOOT_FROM_SDMMC == 1)
/* Note the offset is relative the raw partition beginning, which is at 0x100000 absolutely
 * BARE_METAL : 0x200000(absolute)
 * RBF: 0x300000(absolute)
 *
 * BARE_METAL: 0x200000 - 0x100000 = 0x100000(relative) 
 * RBF: 0x300000 - 0x100000 = 0x200000(relative)
 */
#define CONFIG_PRELOADER_SDMMC_BARE_METAL_IMAGE	(0x100000)
#define CONFIG_SYS_MMCSD_RAW_MODE_BARE_METAL_SECTOR \
	(CONFIG_PRELOADER_SDMMC_BARE_METAL_IMAGE / 512)
#define CONFIG_PRELOADER_SDMMC_BARE_METAL_IMAGE_SIZE	(0x20000)
